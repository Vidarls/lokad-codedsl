//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     ANTLR Version: 3.4.1.9004
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

// $ANTLR 3.4.1.9004 .\\MessageContracts.g 2013-01-25 11:35:48

// The variable 'variable' is assigned but its value is never used.
#pragma warning disable 168, 219
// Unreachable code detected.
#pragma warning disable 162
// Missing XML comment for publicly visible type or member 'Type_or_Member'
#pragma warning disable 1591


using System.Collections.Generic;
using Antlr.Runtime;
using Antlr.Runtime.Misc;
using ConditionalAttribute = System.Diagnostics.ConditionalAttribute;

namespace  MessageContracts 
{
[System.CodeDom.Compiler.GeneratedCode("ANTLR", "3.4.1.9004")]
[System.CLSCompliant(false)]
public partial class MessageContractsLexer : Antlr.Runtime.Lexer
{
	public const int EOF=-1;
	public const int ALIAS=4;
	public const int BlockToken=5;
	public const int CLASS=6;
	public const int COMMENT=7;
	public const int CONST=8;
	public const int ComGuidToken=9;
	public const int ComInterfaceGuidToken=10;
	public const int CommandToken=11;
	public const int DisctionaryToken=12;
	public const int ESC_SEQ=13;
	public const int EXPLICIT=14;
	public const int EXTERN=15;
	public const int EntityDefinition=16;
	public const int EventToken=17;
	public const int ExternToken=18;
	public const int FragmentEntry=19;
	public const int FragmentGroup=20;
	public const int FragmentReference=21;
	public const int GUID=22;
	public const int HEX_DIGIT=23;
	public const int ID=24;
	public const int IF=25;
	public const int INT=26;
	public const int INTERFACE=27;
	public const int MemberToken=28;
	public const int Modifier=29;
	public const int ModifierDefinition=30;
	public const int NAMESPACE=31;
	public const int NORDIC=32;
	public const int NamespaceToken=33;
	public const int OCTAL_ESC=34;
	public const int STRING=35;
	public const int StringRepresentationToken=36;
	public const int TypeToken=37;
	public const int UNICODE_ESC=38;
	public const int USING=39;
	public const int UsingToken=40;
	public const int WS=41;
	public const int T__42=42;
	public const int T__43=43;
	public const int T__44=44;
	public const int T__45=45;
	public const int T__46=46;
	public const int T__47=47;
	public const int T__48=48;
	public const int T__49=49;

    // delegates
    // delegators

	public MessageContractsLexer()
	{
		OnCreated();
	}

	public MessageContractsLexer(ICharStream input )
		: this(input, new RecognizerSharedState())
	{
	}

	public MessageContractsLexer(ICharStream input, RecognizerSharedState state)
		: base(input, state)
	{

		OnCreated();
	}
	public override string GrammarFileName { get { return ".\\MessageContracts.g"; } }

	private static readonly bool[] decisionCanBacktrack = new bool[0];

	[Conditional("ANTLR_TRACE")]
	protected virtual void OnCreated() {}
	[Conditional("ANTLR_TRACE")]
	protected virtual void EnterRule(string ruleName, int ruleIndex) {}
	[Conditional("ANTLR_TRACE")]
	protected virtual void LeaveRule(string ruleName, int ruleIndex) {}

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_T__42() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_T__42() {}

    // $ANTLR start "T__42"
    [GrammarRule("T__42")]
    private void mT__42()
    {
    	EnterRule_T__42();
    	EnterRule("T__42", 1);
    	TraceIn("T__42", 1);
    		try
    		{
    		int _type = T__42;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:9:7: ( '(' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:9:9: '('
    		{
    		DebugLocation(9, 9);
    		Match('('); 

    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("T__42", 1);
    		LeaveRule("T__42", 1);
    		LeaveRule_T__42();
        }
    }
    // $ANTLR end "T__42"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_T__43() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_T__43() {}

    // $ANTLR start "T__43"
    [GrammarRule("T__43")]
    private void mT__43()
    {
    	EnterRule_T__43();
    	EnterRule("T__43", 2);
    	TraceIn("T__43", 2);
    		try
    		{
    		int _type = T__43;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:10:7: ( ')' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:10:9: ')'
    		{
    		DebugLocation(10, 9);
    		Match(')'); 

    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("T__43", 2);
    		LeaveRule("T__43", 2);
    		LeaveRule_T__43();
        }
    }
    // $ANTLR end "T__43"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_T__44() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_T__44() {}

    // $ANTLR start "T__44"
    [GrammarRule("T__44")]
    private void mT__44()
    {
    	EnterRule_T__44();
    	EnterRule("T__44", 3);
    	TraceIn("T__44", 3);
    		try
    		{
    		int _type = T__44;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:11:7: ( ',' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:11:9: ','
    		{
    		DebugLocation(11, 9);
    		Match(','); 

    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("T__44", 3);
    		LeaveRule("T__44", 3);
    		LeaveRule_T__44();
        }
    }
    // $ANTLR end "T__44"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_T__45() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_T__45() {}

    // $ANTLR start "T__45"
    [GrammarRule("T__45")]
    private void mT__45()
    {
    	EnterRule_T__45();
    	EnterRule("T__45", 4);
    	TraceIn("T__45", 4);
    		try
    		{
    		int _type = T__45;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:12:7: ( '.' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:12:9: '.'
    		{
    		DebugLocation(12, 9);
    		Match('.'); 

    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("T__45", 4);
    		LeaveRule("T__45", 4);
    		LeaveRule_T__45();
        }
    }
    // $ANTLR end "T__45"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_T__46() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_T__46() {}

    // $ANTLR start "T__46"
    [GrammarRule("T__46")]
    private void mT__46()
    {
    	EnterRule_T__46();
    	EnterRule("T__46", 5);
    	TraceIn("T__46", 5);
    		try
    		{
    		int _type = T__46;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:13:7: ( ';' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:13:9: ';'
    		{
    		DebugLocation(13, 9);
    		Match(';'); 

    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("T__46", 5);
    		LeaveRule("T__46", 5);
    		LeaveRule_T__46();
        }
    }
    // $ANTLR end "T__46"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_T__47() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_T__47() {}

    // $ANTLR start "T__47"
    [GrammarRule("T__47")]
    private void mT__47()
    {
    	EnterRule_T__47();
    	EnterRule("T__47", 6);
    	TraceIn("T__47", 6);
    		try
    		{
    		int _type = T__47;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:14:7: ( '=' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:14:9: '='
    		{
    		DebugLocation(14, 9);
    		Match('='); 

    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("T__47", 6);
    		LeaveRule("T__47", 6);
    		LeaveRule_T__47();
        }
    }
    // $ANTLR end "T__47"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_T__48() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_T__48() {}

    // $ANTLR start "T__48"
    [GrammarRule("T__48")]
    private void mT__48()
    {
    	EnterRule_T__48();
    	EnterRule("T__48", 7);
    	TraceIn("T__48", 7);
    		try
    		{
    		int _type = T__48;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:15:7: ( '{' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:15:9: '{'
    		{
    		DebugLocation(15, 9);
    		Match('{'); 

    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("T__48", 7);
    		LeaveRule("T__48", 7);
    		LeaveRule_T__48();
        }
    }
    // $ANTLR end "T__48"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_T__49() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_T__49() {}

    // $ANTLR start "T__49"
    [GrammarRule("T__49")]
    private void mT__49()
    {
    	EnterRule_T__49();
    	EnterRule("T__49", 8);
    	TraceIn("T__49", 8);
    		try
    		{
    		int _type = T__49;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:16:7: ( '}' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:16:9: '}'
    		{
    		DebugLocation(16, 9);
    		Match('}'); 

    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("T__49", 8);
    		LeaveRule("T__49", 8);
    		LeaveRule_T__49();
        }
    }
    // $ANTLR end "T__49"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_NORDIC() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_NORDIC() {}

    // $ANTLR start "NORDIC"
    [GrammarRule("NORDIC")]
    private void mNORDIC()
    {
    	EnterRule_NORDIC();
    	EnterRule("NORDIC", 9);
    	TraceIn("NORDIC", 9);
    		try
    		{
    		int _type = NORDIC;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:94:8: ( ( '\\u00E6' | '\\u00C6' | '\\u00F8' | '\\u00D8' | '\\u00E5' | '\\u00C5' ) )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:
    		{
    		DebugLocation(94, 8);
    		if ((input.LA(1)>='\u00C5' && input.LA(1)<='\u00C6')||input.LA(1)=='\u00D8'||(input.LA(1)>='\u00E5' && input.LA(1)<='\u00E6')||input.LA(1)=='\u00F8')
    		{
    			input.Consume();
    		}
    		else
    		{
    			MismatchedSetException mse = new MismatchedSetException(null,input);
    			DebugRecognitionException(mse);
    			Recover(mse);
    			throw mse;
    		}


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("NORDIC", 9);
    		LeaveRule("NORDIC", 9);
    		LeaveRule_NORDIC();
        }
    }
    // $ANTLR end "NORDIC"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_EXPLICIT() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_EXPLICIT() {}

    // $ANTLR start "EXPLICIT"
    [GrammarRule("EXPLICIT")]
    private void mEXPLICIT()
    {
    	EnterRule_EXPLICIT();
    	EnterRule("EXPLICIT", 10);
    	TraceIn("EXPLICIT", 10);
    		try
    		{
    		int _type = EXPLICIT;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:97:2: ( 'explicit' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:97:4: 'explicit'
    		{
    		DebugLocation(97, 4);
    		Match("explicit"); 


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("EXPLICIT", 10);
    		LeaveRule("EXPLICIT", 10);
    		LeaveRule_EXPLICIT();
        }
    }
    // $ANTLR end "EXPLICIT"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_IF() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_IF() {}

    // $ANTLR start "IF"
    [GrammarRule("IF")]
    private void mIF()
    {
    	EnterRule_IF();
    	EnterRule("IF", 11);
    	TraceIn("IF", 11);
    		try
    		{
    		int _type = IF;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:99:2: ( 'if' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:99:4: 'if'
    		{
    		DebugLocation(99, 4);
    		Match("if"); 


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("IF", 11);
    		LeaveRule("IF", 11);
    		LeaveRule_IF();
        }
    }
    // $ANTLR end "IF"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_USING() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_USING() {}

    // $ANTLR start "USING"
    [GrammarRule("USING")]
    private void mUSING()
    {
    	EnterRule_USING();
    	EnterRule("USING", 12);
    	TraceIn("USING", 12);
    		try
    		{
    		int _type = USING;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:101:5: ( 'using' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:101:7: 'using'
    		{
    		DebugLocation(101, 7);
    		Match("using"); 


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("USING", 12);
    		LeaveRule("USING", 12);
    		LeaveRule_USING();
        }
    }
    // $ANTLR end "USING"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_CONST() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_CONST() {}

    // $ANTLR start "CONST"
    [GrammarRule("CONST")]
    private void mCONST()
    {
    	EnterRule_CONST();
    	EnterRule("CONST", 13);
    	TraceIn("CONST", 13);
    		try
    		{
    		int _type = CONST;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:103:2: ( 'const' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:103:4: 'const'
    		{
    		DebugLocation(103, 4);
    		Match("const"); 


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("CONST", 13);
    		LeaveRule("CONST", 13);
    		LeaveRule_CONST();
        }
    }
    // $ANTLR end "CONST"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_INTERFACE() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_INTERFACE() {}

    // $ANTLR start "INTERFACE"
    [GrammarRule("INTERFACE")]
    private void mINTERFACE()
    {
    	EnterRule_INTERFACE();
    	EnterRule("INTERFACE", 14);
    	TraceIn("INTERFACE", 14);
    		try
    		{
    		int _type = INTERFACE;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:105:2: ( 'interface' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:105:4: 'interface'
    		{
    		DebugLocation(105, 4);
    		Match("interface"); 


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("INTERFACE", 14);
    		LeaveRule("INTERFACE", 14);
    		LeaveRule_INTERFACE();
        }
    }
    // $ANTLR end "INTERFACE"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_CLASS() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_CLASS() {}

    // $ANTLR start "CLASS"
    [GrammarRule("CLASS")]
    private void mCLASS()
    {
    	EnterRule_CLASS();
    	EnterRule("CLASS", 15);
    	TraceIn("CLASS", 15);
    		try
    		{
    		int _type = CLASS;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:107:2: ( 'class' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:107:4: 'class'
    		{
    		DebugLocation(107, 4);
    		Match("class"); 


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("CLASS", 15);
    		LeaveRule("CLASS", 15);
    		LeaveRule_CLASS();
        }
    }
    // $ANTLR end "CLASS"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_ALIAS() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_ALIAS() {}

    // $ANTLR start "ALIAS"
    [GrammarRule("ALIAS")]
    private void mALIAS()
    {
    	EnterRule_ALIAS();
    	EnterRule("ALIAS", 16);
    	TraceIn("ALIAS", 16);
    		try
    		{
    		int _type = ALIAS;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:109:2: ( 'alias' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:109:4: 'alias'
    		{
    		DebugLocation(109, 4);
    		Match("alias"); 


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("ALIAS", 16);
    		LeaveRule("ALIAS", 16);
    		LeaveRule_ALIAS();
        }
    }
    // $ANTLR end "ALIAS"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_NAMESPACE() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_NAMESPACE() {}

    // $ANTLR start "NAMESPACE"
    [GrammarRule("NAMESPACE")]
    private void mNAMESPACE()
    {
    	EnterRule_NAMESPACE();
    	EnterRule("NAMESPACE", 17);
    	TraceIn("NAMESPACE", 17);
    		try
    		{
    		int _type = NAMESPACE;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:111:2: ( 'namespace' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:111:4: 'namespace'
    		{
    		DebugLocation(111, 4);
    		Match("namespace"); 


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("NAMESPACE", 17);
    		LeaveRule("NAMESPACE", 17);
    		LeaveRule_NAMESPACE();
        }
    }
    // $ANTLR end "NAMESPACE"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_EXTERN() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_EXTERN() {}

    // $ANTLR start "EXTERN"
    [GrammarRule("EXTERN")]
    private void mEXTERN()
    {
    	EnterRule_EXTERN();
    	EnterRule("EXTERN", 18);
    	TraceIn("EXTERN", 18);
    		try
    		{
    		int _type = EXTERN;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:113:5: ( 'extern' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:113:7: 'extern'
    		{
    		DebugLocation(113, 7);
    		Match("extern"); 


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("EXTERN", 18);
    		LeaveRule("EXTERN", 18);
    		LeaveRule_EXTERN();
        }
    }
    // $ANTLR end "EXTERN"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_ID() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_ID() {}

    // $ANTLR start "ID"
    [GrammarRule("ID")]
    private void mID()
    {
    	EnterRule_ID();
    	EnterRule("ID", 19);
    	TraceIn("ID", 19);
    		try
    		{
    		int _type = ID;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:115:5: ( ( 'a' .. 'z' | 'A' .. 'Z' | '_' | NORDIC ) ( 'a' .. 'z' | 'A' .. 'Z' | NORDIC | '0' .. '9' | '_' | '<' | '>' | '[' | ']' )* )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:115:7: ( 'a' .. 'z' | 'A' .. 'Z' | '_' | NORDIC ) ( 'a' .. 'z' | 'A' .. 'Z' | NORDIC | '0' .. '9' | '_' | '<' | '>' | '[' | ']' )*
    		{
    		DebugLocation(115, 7);
    		if ((input.LA(1)>='A' && input.LA(1)<='Z')||input.LA(1)=='_'||(input.LA(1)>='a' && input.LA(1)<='z')||(input.LA(1)>='\u00C5' && input.LA(1)<='\u00C6')||input.LA(1)=='\u00D8'||(input.LA(1)>='\u00E5' && input.LA(1)<='\u00E6')||input.LA(1)=='\u00F8')
    		{
    			input.Consume();
    		}
    		else
    		{
    			MismatchedSetException mse = new MismatchedSetException(null,input);
    			DebugRecognitionException(mse);
    			Recover(mse);
    			throw mse;
    		}

    		DebugLocation(115, 38);
    		// .\\MessageContracts.g:115:38: ( 'a' .. 'z' | 'A' .. 'Z' | NORDIC | '0' .. '9' | '_' | '<' | '>' | '[' | ']' )*
    		try { DebugEnterSubRule(1);
    		while (true)
    		{
    			int alt1=2;
    			try { DebugEnterDecision(1, decisionCanBacktrack[1]);
    			int LA1_1 = input.LA(1);

    			if (((LA1_1>='0' && LA1_1<='9')||LA1_1=='<'||LA1_1=='>'||(LA1_1>='A' && LA1_1<='[')||LA1_1==']'||LA1_1=='_'||(LA1_1>='a' && LA1_1<='z')||(LA1_1>='\u00C5' && LA1_1<='\u00C6')||LA1_1=='\u00D8'||(LA1_1>='\u00E5' && LA1_1<='\u00E6')||LA1_1=='\u00F8'))
    			{
    				alt1 = 1;
    			}


    			} finally { DebugExitDecision(1); }
    			switch ( alt1 )
    			{
    			case 1:
    				DebugEnterAlt(1);
    				// .\\MessageContracts.g:
    				{
    				DebugLocation(115, 38);
    				input.Consume();


    				}
    				break;

    			default:
    				goto loop1;
    			}
    		}

    		loop1:
    			;

    		} finally { DebugExitSubRule(1); }


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("ID", 19);
    		LeaveRule("ID", 19);
    		LeaveRule_ID();
        }
    }
    // $ANTLR end "ID"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_GUID() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_GUID() {}

    // $ANTLR start "GUID"
    [GrammarRule("GUID")]
    private void mGUID()
    {
    	EnterRule_GUID();
    	EnterRule("GUID", 20);
    	TraceIn("GUID", 20);
    		try
    		{
    		int _type = GUID;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:119:2: ( '[Guid(\"' ( HEX_DIGIT | '-' )+ '\")]' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:119:4: '[Guid(\"' ( HEX_DIGIT | '-' )+ '\")]'
    		{
    		DebugLocation(119, 4);
    		Match("[Guid(\""); 

    		DebugLocation(119, 14);
    		// .\\MessageContracts.g:119:14: ( HEX_DIGIT | '-' )+
    		int cnt2=0;
    		try { DebugEnterSubRule(2);
    		while (true)
    		{
    			int alt2=2;
    			try { DebugEnterDecision(2, decisionCanBacktrack[2]);
    			int LA2_1 = input.LA(1);

    			if ((LA2_1=='-'||(LA2_1>='0' && LA2_1<='9')||(LA2_1>='A' && LA2_1<='F')||(LA2_1>='a' && LA2_1<='f')))
    			{
    				alt2 = 1;
    			}


    			} finally { DebugExitDecision(2); }
    			switch (alt2)
    			{
    			case 1:
    				DebugEnterAlt(1);
    				// .\\MessageContracts.g:
    				{
    				DebugLocation(119, 14);
    				input.Consume();


    				}
    				break;

    			default:
    				if (cnt2 >= 1)
    					goto loop2;

    				EarlyExitException eee2 = new EarlyExitException( 2, input );
    				DebugRecognitionException(eee2);
    				throw eee2;
    			}
    			cnt2++;
    		}
    		loop2:
    			;

    		} finally { DebugExitSubRule(2); }

    		DebugLocation(119, 31);
    		Match("\")]"); 


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("GUID", 20);
    		LeaveRule("GUID", 20);
    		LeaveRule_GUID();
        }
    }
    // $ANTLR end "GUID"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_Modifier() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_Modifier() {}

    // $ANTLR start "Modifier"
    [GrammarRule("Modifier")]
    private void mModifier()
    {
    	EnterRule_Modifier();
    	EnterRule("Modifier", 21);
    	TraceIn("Modifier", 21);
    		try
    		{
    		int _type = Modifier;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:122:2: ( '?' | '!' | ';' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:
    		{
    		DebugLocation(122, 2);
    		if (input.LA(1)=='!'||input.LA(1)==';'||input.LA(1)=='?')
    		{
    			input.Consume();
    		}
    		else
    		{
    			MismatchedSetException mse = new MismatchedSetException(null,input);
    			DebugRecognitionException(mse);
    			Recover(mse);
    			throw mse;
    		}


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("Modifier", 21);
    		LeaveRule("Modifier", 21);
    		LeaveRule_Modifier();
        }
    }
    // $ANTLR end "Modifier"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_INT() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_INT() {}

    // $ANTLR start "INT"
    [GrammarRule("INT")]
    private void mINT()
    {
    	EnterRule_INT();
    	EnterRule("INT", 22);
    	TraceIn("INT", 22);
    		try
    		{
    		int _type = INT;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:128:5: ( ( '0' .. '9' )+ )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:128:7: ( '0' .. '9' )+
    		{
    		DebugLocation(128, 7);
    		// .\\MessageContracts.g:128:7: ( '0' .. '9' )+
    		int cnt3=0;
    		try { DebugEnterSubRule(3);
    		while (true)
    		{
    			int alt3=2;
    			try { DebugEnterDecision(3, decisionCanBacktrack[3]);
    			int LA3_1 = input.LA(1);

    			if (((LA3_1>='0' && LA3_1<='9')))
    			{
    				alt3 = 1;
    			}


    			} finally { DebugExitDecision(3); }
    			switch (alt3)
    			{
    			case 1:
    				DebugEnterAlt(1);
    				// .\\MessageContracts.g:
    				{
    				DebugLocation(128, 7);
    				input.Consume();


    				}
    				break;

    			default:
    				if (cnt3 >= 1)
    					goto loop3;

    				EarlyExitException eee3 = new EarlyExitException( 3, input );
    				DebugRecognitionException(eee3);
    				throw eee3;
    			}
    			cnt3++;
    		}
    		loop3:
    			;

    		} finally { DebugExitSubRule(3); }


    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("INT", 22);
    		LeaveRule("INT", 22);
    		LeaveRule_INT();
        }
    }
    // $ANTLR end "INT"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_STRING() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_STRING() {}

    // $ANTLR start "STRING"
    [GrammarRule("STRING")]
    private void mSTRING()
    {
    	EnterRule_STRING();
    	EnterRule("STRING", 23);
    	TraceIn("STRING", 23);
    		try
    		{
    		int _type = STRING;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:132:5: ( '\"' ( ESC_SEQ |~ ( '\\\\' | '\"' ) )* '\"' )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:132:8: '\"' ( ESC_SEQ |~ ( '\\\\' | '\"' ) )* '\"'
    		{
    		DebugLocation(132, 8);
    		Match('\"'); 
    		DebugLocation(132, 12);
    		// .\\MessageContracts.g:132:12: ( ESC_SEQ |~ ( '\\\\' | '\"' ) )*
    		try { DebugEnterSubRule(4);
    		while (true)
    		{
    			int alt4=3;
    			try { DebugEnterDecision(4, decisionCanBacktrack[4]);
    			int LA4_1 = input.LA(1);

    			if ((LA4_1=='\\'))
    			{
    				alt4 = 1;
    			}
    			else if (((LA4_1>='\u0000' && LA4_1<='!')||(LA4_1>='#' && LA4_1<='[')||(LA4_1>=']' && LA4_1<='\uFFFF')))
    			{
    				alt4 = 2;
    			}


    			} finally { DebugExitDecision(4); }
    			switch ( alt4 )
    			{
    			case 1:
    				DebugEnterAlt(1);
    				// .\\MessageContracts.g:132:14: ESC_SEQ
    				{
    				DebugLocation(132, 14);
    				mESC_SEQ(); 

    				}
    				break;
    			case 2:
    				DebugEnterAlt(2);
    				// .\\MessageContracts.g:132:24: ~ ( '\\\\' | '\"' )
    				{
    				DebugLocation(132, 24);
    				input.Consume();


    				}
    				break;

    			default:
    				goto loop4;
    			}
    		}

    		loop4:
    			;

    		} finally { DebugExitSubRule(4); }

    		DebugLocation(132, 39);
    		Match('\"'); 

    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("STRING", 23);
    		LeaveRule("STRING", 23);
    		LeaveRule_STRING();
        }
    }
    // $ANTLR end "STRING"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_HEX_DIGIT() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_HEX_DIGIT() {}

    // $ANTLR start "HEX_DIGIT"
    [GrammarRule("HEX_DIGIT")]
    private void mHEX_DIGIT()
    {
    	EnterRule_HEX_DIGIT();
    	EnterRule("HEX_DIGIT", 24);
    	TraceIn("HEX_DIGIT", 24);
    		try
    		{
    		// .\\MessageContracts.g:138:11: ( ( '0' .. '9' | 'a' .. 'f' | 'A' .. 'F' ) )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:
    		{
    		DebugLocation(138, 11);
    		if ((input.LA(1)>='0' && input.LA(1)<='9')||(input.LA(1)>='A' && input.LA(1)<='F')||(input.LA(1)>='a' && input.LA(1)<='f'))
    		{
    			input.Consume();
    		}
    		else
    		{
    			MismatchedSetException mse = new MismatchedSetException(null,input);
    			DebugRecognitionException(mse);
    			Recover(mse);
    			throw mse;
    		}


    		}

    	}
    	finally
    	{
    		TraceOut("HEX_DIGIT", 24);
    		LeaveRule("HEX_DIGIT", 24);
    		LeaveRule_HEX_DIGIT();
        }
    }
    // $ANTLR end "HEX_DIGIT"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_ESC_SEQ() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_ESC_SEQ() {}

    // $ANTLR start "ESC_SEQ"
    [GrammarRule("ESC_SEQ")]
    private void mESC_SEQ()
    {
    	EnterRule_ESC_SEQ();
    	EnterRule("ESC_SEQ", 25);
    	TraceIn("ESC_SEQ", 25);
    		try
    		{
    		// .\\MessageContracts.g:142:5: ( '\\\\' ( 'b' | 't' | 'n' | 'f' | 'r' | '\\\"' | '\\'' | '\\\\' ) | UNICODE_ESC | OCTAL_ESC )
    		int alt5=3;
    		try { DebugEnterDecision(5, decisionCanBacktrack[5]);
    		int LA5_1 = input.LA(1);

    		if ((LA5_1=='\\'))
    		{
    			switch (input.LA(2))
    			{
    			case '\"':
    			case '\'':
    			case '\\':
    			case 'b':
    			case 'f':
    			case 'n':
    			case 'r':
    			case 't':
    				{
    				alt5 = 1;
    				}
    				break;
    			case 'u':
    				{
    				alt5 = 2;
    				}
    				break;
    			case '0':
    			case '1':
    			case '2':
    			case '3':
    			case '4':
    			case '5':
    			case '6':
    			case '7':
    				{
    				alt5 = 3;
    				}
    				break;
    			default:
    				{
    					NoViableAltException nvae = new NoViableAltException("", 5, 1, input, 2);
    					DebugRecognitionException(nvae);
    					throw nvae;
    				}
    			}

    		}
    		else
    		{
    			NoViableAltException nvae = new NoViableAltException("", 5, 0, input, 1);
    			DebugRecognitionException(nvae);
    			throw nvae;
    		}
    		} finally { DebugExitDecision(5); }
    		switch (alt5)
    		{
    		case 1:
    			DebugEnterAlt(1);
    			// .\\MessageContracts.g:142:9: '\\\\' ( 'b' | 't' | 'n' | 'f' | 'r' | '\\\"' | '\\'' | '\\\\' )
    			{
    			DebugLocation(142, 9);
    			Match('\\'); 
    			DebugLocation(142, 14);
    			input.Consume();


    			}
    			break;
    		case 2:
    			DebugEnterAlt(2);
    			// .\\MessageContracts.g:143:9: UNICODE_ESC
    			{
    			DebugLocation(143, 9);
    			mUNICODE_ESC(); 

    			}
    			break;
    		case 3:
    			DebugEnterAlt(3);
    			// .\\MessageContracts.g:144:9: OCTAL_ESC
    			{
    			DebugLocation(144, 9);
    			mOCTAL_ESC(); 

    			}
    			break;

    		}
    	}
    	finally
    	{
    		TraceOut("ESC_SEQ", 25);
    		LeaveRule("ESC_SEQ", 25);
    		LeaveRule_ESC_SEQ();
        }
    }
    // $ANTLR end "ESC_SEQ"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_OCTAL_ESC() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_OCTAL_ESC() {}

    // $ANTLR start "OCTAL_ESC"
    [GrammarRule("OCTAL_ESC")]
    private void mOCTAL_ESC()
    {
    	EnterRule_OCTAL_ESC();
    	EnterRule("OCTAL_ESC", 26);
    	TraceIn("OCTAL_ESC", 26);
    		try
    		{
    		// .\\MessageContracts.g:149:5: ( '\\\\' ( '0' .. '3' ) ( '0' .. '7' ) ( '0' .. '7' ) | '\\\\' ( '0' .. '7' ) ( '0' .. '7' ) | '\\\\' ( '0' .. '7' ) )
    		int alt6=3;
    		try { DebugEnterDecision(6, decisionCanBacktrack[6]);
    		int LA6_1 = input.LA(1);

    		if ((LA6_1=='\\'))
    		{
    			int LA6_2 = input.LA(2);

    			if (((LA6_2>='0' && LA6_2<='3')))
    			{
    				int LA6_3 = input.LA(3);

    				if (((LA6_3>='0' && LA6_3<='7')))
    				{
    					int LA6_4 = input.LA(4);

    					if (((LA6_4>='0' && LA6_4<='7')))
    					{
    						alt6 = 1;
    					}
    					else
    					{
    						alt6 = 2;
    					}
    				}
    				else
    				{
    					alt6 = 3;
    				}
    			}
    			else if (((LA6_2>='4' && LA6_2<='7')))
    			{
    				int LA6_3 = input.LA(3);

    				if (((LA6_3>='0' && LA6_3<='7')))
    				{
    					alt6 = 2;
    				}
    				else
    				{
    					alt6 = 3;
    				}
    			}
    			else
    			{
    				NoViableAltException nvae = new NoViableAltException("", 6, 1, input, 2);
    				DebugRecognitionException(nvae);
    				throw nvae;
    			}
    		}
    		else
    		{
    			NoViableAltException nvae = new NoViableAltException("", 6, 0, input, 1);
    			DebugRecognitionException(nvae);
    			throw nvae;
    		}
    		} finally { DebugExitDecision(6); }
    		switch (alt6)
    		{
    		case 1:
    			DebugEnterAlt(1);
    			// .\\MessageContracts.g:149:9: '\\\\' ( '0' .. '3' ) ( '0' .. '7' ) ( '0' .. '7' )
    			{
    			DebugLocation(149, 9);
    			Match('\\'); 
    			DebugLocation(149, 14);
    			input.Consume();

    			DebugLocation(149, 25);
    			input.Consume();

    			DebugLocation(149, 36);
    			input.Consume();


    			}
    			break;
    		case 2:
    			DebugEnterAlt(2);
    			// .\\MessageContracts.g:150:9: '\\\\' ( '0' .. '7' ) ( '0' .. '7' )
    			{
    			DebugLocation(150, 9);
    			Match('\\'); 
    			DebugLocation(150, 14);
    			input.Consume();

    			DebugLocation(150, 25);
    			input.Consume();


    			}
    			break;
    		case 3:
    			DebugEnterAlt(3);
    			// .\\MessageContracts.g:151:9: '\\\\' ( '0' .. '7' )
    			{
    			DebugLocation(151, 9);
    			Match('\\'); 
    			DebugLocation(151, 14);
    			input.Consume();


    			}
    			break;

    		}
    	}
    	finally
    	{
    		TraceOut("OCTAL_ESC", 26);
    		LeaveRule("OCTAL_ESC", 26);
    		LeaveRule_OCTAL_ESC();
        }
    }
    // $ANTLR end "OCTAL_ESC"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_UNICODE_ESC() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_UNICODE_ESC() {}

    // $ANTLR start "UNICODE_ESC"
    [GrammarRule("UNICODE_ESC")]
    private void mUNICODE_ESC()
    {
    	EnterRule_UNICODE_ESC();
    	EnterRule("UNICODE_ESC", 27);
    	TraceIn("UNICODE_ESC", 27);
    		try
    		{
    		// .\\MessageContracts.g:156:5: ( '\\\\' 'u' HEX_DIGIT HEX_DIGIT HEX_DIGIT HEX_DIGIT )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:156:9: '\\\\' 'u' HEX_DIGIT HEX_DIGIT HEX_DIGIT HEX_DIGIT
    		{
    		DebugLocation(156, 9);
    		Match('\\'); 
    		DebugLocation(156, 14);
    		Match('u'); 
    		DebugLocation(156, 18);
    		mHEX_DIGIT(); 
    		DebugLocation(156, 28);
    		mHEX_DIGIT(); 
    		DebugLocation(156, 38);
    		mHEX_DIGIT(); 
    		DebugLocation(156, 48);
    		mHEX_DIGIT(); 

    		}

    	}
    	finally
    	{
    		TraceOut("UNICODE_ESC", 27);
    		LeaveRule("UNICODE_ESC", 27);
    		LeaveRule_UNICODE_ESC();
        }
    }
    // $ANTLR end "UNICODE_ESC"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_COMMENT() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_COMMENT() {}

    // $ANTLR start "COMMENT"
    [GrammarRule("COMMENT")]
    private void mCOMMENT()
    {
    	EnterRule_COMMENT();
    	EnterRule("COMMENT", 28);
    	TraceIn("COMMENT", 28);
    		try
    		{
    		int _type = COMMENT;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:159:5: ( '//' (~ ( '\\n' | '\\r' ) )* ( '\\r' )? '\\n' | '/*' ( options {greedy=false; } : . )* '*/' )
    		int alt10=2;
    		try { DebugEnterDecision(10, decisionCanBacktrack[10]);
    		int LA10_1 = input.LA(1);

    		if ((LA10_1=='/'))
    		{
    			int LA10_2 = input.LA(2);

    			if ((LA10_2=='/'))
    			{
    				alt10 = 1;
    			}
    			else if ((LA10_2=='*'))
    			{
    				alt10 = 2;
    			}
    			else
    			{
    				NoViableAltException nvae = new NoViableAltException("", 10, 1, input, 2);
    				DebugRecognitionException(nvae);
    				throw nvae;
    			}
    		}
    		else
    		{
    			NoViableAltException nvae = new NoViableAltException("", 10, 0, input, 1);
    			DebugRecognitionException(nvae);
    			throw nvae;
    		}
    		} finally { DebugExitDecision(10); }
    		switch (alt10)
    		{
    		case 1:
    			DebugEnterAlt(1);
    			// .\\MessageContracts.g:159:9: '//' (~ ( '\\n' | '\\r' ) )* ( '\\r' )? '\\n'
    			{
    			DebugLocation(159, 9);
    			Match("//"); 

    			DebugLocation(159, 14);
    			// .\\MessageContracts.g:159:14: (~ ( '\\n' | '\\r' ) )*
    			try { DebugEnterSubRule(7);
    			while (true)
    			{
    				int alt7=2;
    				try { DebugEnterDecision(7, decisionCanBacktrack[7]);
    				int LA7_1 = input.LA(1);

    				if (((LA7_1>='\u0000' && LA7_1<='\t')||(LA7_1>='\u000B' && LA7_1<='\f')||(LA7_1>='\u000E' && LA7_1<='\uFFFF')))
    				{
    					alt7 = 1;
    				}


    				} finally { DebugExitDecision(7); }
    				switch ( alt7 )
    				{
    				case 1:
    					DebugEnterAlt(1);
    					// .\\MessageContracts.g:
    					{
    					DebugLocation(159, 14);
    					input.Consume();


    					}
    					break;

    				default:
    					goto loop7;
    				}
    			}

    			loop7:
    				;

    			} finally { DebugExitSubRule(7); }

    			DebugLocation(159, 28);
    			// .\\MessageContracts.g:159:28: ( '\\r' )?
    			int alt8=2;
    			try { DebugEnterSubRule(8);
    			try { DebugEnterDecision(8, decisionCanBacktrack[8]);
    			int LA8_1 = input.LA(1);

    			if ((LA8_1=='\r'))
    			{
    				alt8 = 1;
    			}
    			} finally { DebugExitDecision(8); }
    			switch (alt8)
    			{
    			case 1:
    				DebugEnterAlt(1);
    				// .\\MessageContracts.g:159:28: '\\r'
    				{
    				DebugLocation(159, 28);
    				Match('\r'); 

    				}
    				break;

    			}
    			} finally { DebugExitSubRule(8); }

    			DebugLocation(159, 34);
    			Match('\n'); 
    			DebugLocation(159, 39);
    			_channel=HIDDEN;

    			}
    			break;
    		case 2:
    			DebugEnterAlt(2);
    			// .\\MessageContracts.g:160:9: '/*' ( options {greedy=false; } : . )* '*/'
    			{
    			DebugLocation(160, 9);
    			Match("/*"); 

    			DebugLocation(160, 14);
    			// .\\MessageContracts.g:160:14: ( options {greedy=false; } : . )*
    			try { DebugEnterSubRule(9);
    			while (true)
    			{
    				int alt9=2;
    				try { DebugEnterDecision(9, decisionCanBacktrack[9]);
    				int LA9_1 = input.LA(1);

    				if ((LA9_1=='*'))
    				{
    					int LA9_2 = input.LA(2);

    					if ((LA9_2=='/'))
    					{
    						alt9 = 2;
    					}
    					else if (((LA9_2>='\u0000' && LA9_2<='.')||(LA9_2>='0' && LA9_2<='\uFFFF')))
    					{
    						alt9 = 1;
    					}


    				}
    				else if (((LA9_1>='\u0000' && LA9_1<=')')||(LA9_1>='+' && LA9_1<='\uFFFF')))
    				{
    					alt9 = 1;
    				}


    				} finally { DebugExitDecision(9); }
    				switch ( alt9 )
    				{
    				case 1:
    					DebugEnterAlt(1);
    					// .\\MessageContracts.g:160:42: .
    					{
    					DebugLocation(160, 42);
    					MatchAny(); 

    					}
    					break;

    				default:
    					goto loop9;
    				}
    			}

    			loop9:
    				;

    			} finally { DebugExitSubRule(9); }

    			DebugLocation(160, 47);
    			Match("*/"); 

    			DebugLocation(160, 52);
    			_channel=HIDDEN;

    			}
    			break;

    		}
    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("COMMENT", 28);
    		LeaveRule("COMMENT", 28);
    		LeaveRule_COMMENT();
        }
    }
    // $ANTLR end "COMMENT"

    [Conditional("ANTLR_TRACE")]
    protected virtual void EnterRule_WS() {}
    [Conditional("ANTLR_TRACE")]
    protected virtual void LeaveRule_WS() {}

    // $ANTLR start "WS"
    [GrammarRule("WS")]
    private void mWS()
    {
    	EnterRule_WS();
    	EnterRule("WS", 29);
    	TraceIn("WS", 29);
    		try
    		{
    		int _type = WS;
    		int _channel = DefaultTokenChannel;
    		// .\\MessageContracts.g:163:5: ( ( ' ' | '\\t' | '\\r' | '\\n' ) )
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:163:9: ( ' ' | '\\t' | '\\r' | '\\n' )
    		{
    		DebugLocation(163, 9);
    		if ((input.LA(1)>='\t' && input.LA(1)<='\n')||input.LA(1)=='\r'||input.LA(1)==' ')
    		{
    			input.Consume();
    		}
    		else
    		{
    			MismatchedSetException mse = new MismatchedSetException(null,input);
    			DebugRecognitionException(mse);
    			Recover(mse);
    			throw mse;
    		}

    		DebugLocation(167, 11);
    		_channel=HIDDEN;

    		}

    		state.type = _type;
    		state.channel = _channel;
    	}
    	finally
    	{
    		TraceOut("WS", 29);
    		LeaveRule("WS", 29);
    		LeaveRule_WS();
        }
    }
    // $ANTLR end "WS"

    public override void mTokens()
    {
    	// .\\MessageContracts.g:1:8: ( T__42 | T__43 | T__44 | T__45 | T__46 | T__47 | T__48 | T__49 | NORDIC | EXPLICIT | IF | USING | CONST | INTERFACE | CLASS | ALIAS | NAMESPACE | EXTERN | ID | GUID | Modifier | INT | STRING | COMMENT | WS )
    	int alt11=25;
    	try { DebugEnterDecision(11, decisionCanBacktrack[11]);
    	try
    	{
    		alt11 = dfa11.Predict(input);
    	}
    	catch (NoViableAltException nvae)
    	{
    		DebugRecognitionException(nvae);
    		throw;
    	}
    	} finally { DebugExitDecision(11); }
    	switch (alt11)
    	{
    	case 1:
    		DebugEnterAlt(1);
    		// .\\MessageContracts.g:1:10: T__42
    		{
    		DebugLocation(1, 10);
    		mT__42(); 

    		}
    		break;
    	case 2:
    		DebugEnterAlt(2);
    		// .\\MessageContracts.g:1:16: T__43
    		{
    		DebugLocation(1, 16);
    		mT__43(); 

    		}
    		break;
    	case 3:
    		DebugEnterAlt(3);
    		// .\\MessageContracts.g:1:22: T__44
    		{
    		DebugLocation(1, 22);
    		mT__44(); 

    		}
    		break;
    	case 4:
    		DebugEnterAlt(4);
    		// .\\MessageContracts.g:1:28: T__45
    		{
    		DebugLocation(1, 28);
    		mT__45(); 

    		}
    		break;
    	case 5:
    		DebugEnterAlt(5);
    		// .\\MessageContracts.g:1:34: T__46
    		{
    		DebugLocation(1, 34);
    		mT__46(); 

    		}
    		break;
    	case 6:
    		DebugEnterAlt(6);
    		// .\\MessageContracts.g:1:40: T__47
    		{
    		DebugLocation(1, 40);
    		mT__47(); 

    		}
    		break;
    	case 7:
    		DebugEnterAlt(7);
    		// .\\MessageContracts.g:1:46: T__48
    		{
    		DebugLocation(1, 46);
    		mT__48(); 

    		}
    		break;
    	case 8:
    		DebugEnterAlt(8);
    		// .\\MessageContracts.g:1:52: T__49
    		{
    		DebugLocation(1, 52);
    		mT__49(); 

    		}
    		break;
    	case 9:
    		DebugEnterAlt(9);
    		// .\\MessageContracts.g:1:58: NORDIC
    		{
    		DebugLocation(1, 58);
    		mNORDIC(); 

    		}
    		break;
    	case 10:
    		DebugEnterAlt(10);
    		// .\\MessageContracts.g:1:65: EXPLICIT
    		{
    		DebugLocation(1, 65);
    		mEXPLICIT(); 

    		}
    		break;
    	case 11:
    		DebugEnterAlt(11);
    		// .\\MessageContracts.g:1:74: IF
    		{
    		DebugLocation(1, 74);
    		mIF(); 

    		}
    		break;
    	case 12:
    		DebugEnterAlt(12);
    		// .\\MessageContracts.g:1:77: USING
    		{
    		DebugLocation(1, 77);
    		mUSING(); 

    		}
    		break;
    	case 13:
    		DebugEnterAlt(13);
    		// .\\MessageContracts.g:1:83: CONST
    		{
    		DebugLocation(1, 83);
    		mCONST(); 

    		}
    		break;
    	case 14:
    		DebugEnterAlt(14);
    		// .\\MessageContracts.g:1:89: INTERFACE
    		{
    		DebugLocation(1, 89);
    		mINTERFACE(); 

    		}
    		break;
    	case 15:
    		DebugEnterAlt(15);
    		// .\\MessageContracts.g:1:99: CLASS
    		{
    		DebugLocation(1, 99);
    		mCLASS(); 

    		}
    		break;
    	case 16:
    		DebugEnterAlt(16);
    		// .\\MessageContracts.g:1:105: ALIAS
    		{
    		DebugLocation(1, 105);
    		mALIAS(); 

    		}
    		break;
    	case 17:
    		DebugEnterAlt(17);
    		// .\\MessageContracts.g:1:111: NAMESPACE
    		{
    		DebugLocation(1, 111);
    		mNAMESPACE(); 

    		}
    		break;
    	case 18:
    		DebugEnterAlt(18);
    		// .\\MessageContracts.g:1:121: EXTERN
    		{
    		DebugLocation(1, 121);
    		mEXTERN(); 

    		}
    		break;
    	case 19:
    		DebugEnterAlt(19);
    		// .\\MessageContracts.g:1:128: ID
    		{
    		DebugLocation(1, 128);
    		mID(); 

    		}
    		break;
    	case 20:
    		DebugEnterAlt(20);
    		// .\\MessageContracts.g:1:131: GUID
    		{
    		DebugLocation(1, 131);
    		mGUID(); 

    		}
    		break;
    	case 21:
    		DebugEnterAlt(21);
    		// .\\MessageContracts.g:1:136: Modifier
    		{
    		DebugLocation(1, 136);
    		mModifier(); 

    		}
    		break;
    	case 22:
    		DebugEnterAlt(22);
    		// .\\MessageContracts.g:1:145: INT
    		{
    		DebugLocation(1, 145);
    		mINT(); 

    		}
    		break;
    	case 23:
    		DebugEnterAlt(23);
    		// .\\MessageContracts.g:1:149: STRING
    		{
    		DebugLocation(1, 149);
    		mSTRING(); 

    		}
    		break;
    	case 24:
    		DebugEnterAlt(24);
    		// .\\MessageContracts.g:1:156: COMMENT
    		{
    		DebugLocation(1, 156);
    		mCOMMENT(); 

    		}
    		break;
    	case 25:
    		DebugEnterAlt(25);
    		// .\\MessageContracts.g:1:164: WS
    		{
    		DebugLocation(1, 164);
    		mWS(); 

    		}
    		break;

    	}

    }


	#region DFA
	DFA11 dfa11;

	protected override void InitDFAs()
	{
		base.InitDFAs();
		dfa11 = new DFA11(this);
	}

	private class DFA11 : DFA
	{
		private const string DFA11_eotS =
			"\x9\xFFFF\x1\x18\x6\x10\x9\xFFFF\x1\x10\x1\x23\x8\x10\x1\xFFFF\x11\x10"+
			"\x1\x3D\x1\x3E\x1\x3F\x1\x40\x2\x10\x1\x43\x1\x10\x4\xFFFF\x2\x10\x1"+
			"\xFFFF\x2\x10\x1\x49\x2\x10\x1\xFFFF\x1\x4C\x1\x4D\x2\xFFFF";
		private const string DFA11_eofS =
			"\x4E\xFFFF";
		private const string DFA11_minS =
			"\x1\x9\x8\xFFFF\x1\x30\x1\x78\x1\x66\x1\x73\x2\x6C\x1\x61\x9\xFFFF\x1"+
			"\x70\x1\x30\x1\x74\x1\x69\x1\x6E\x1\x61\x1\x69\x1\x6D\x1\x6C\x1\x65\x1"+
			"\xFFFF\x1\x65\x1\x6E\x2\x73\x1\x61\x1\x65\x1\x69\x2\x72\x1\x67\x1\x74"+
			"\x3\x73\x1\x63\x1\x6E\x1\x66\x4\x30\x1\x70\x1\x69\x1\x30\x1\x61\x4\xFFFF"+
			"\x1\x61\x1\x74\x1\xFFFF\x2\x63\x1\x30\x2\x65\x1\xFFFF\x2\x30\x2\xFFFF";
		private const string DFA11_maxS =
			"\x1\xF8\x8\xFFFF\x1\xF8\x1\x78\x1\x6E\x1\x73\x1\x6F\x1\x6C\x1\x61\x9"+
			"\xFFFF\x1\x74\x1\xF8\x1\x74\x1\x69\x1\x6E\x1\x61\x1\x69\x1\x6D\x1\x6C"+
			"\x1\x65\x1\xFFFF\x1\x65\x1\x6E\x2\x73\x1\x61\x1\x65\x1\x69\x2\x72\x1"+
			"\x67\x1\x74\x3\x73\x1\x63\x1\x6E\x1\x66\x4\xF8\x1\x70\x1\x69\x1\xF8\x1"+
			"\x61\x4\xFFFF\x1\x61\x1\x74\x1\xFFFF\x2\x63\x1\xF8\x2\x65\x1\xFFFF\x2"+
			"\xF8\x2\xFFFF";
		private const string DFA11_acceptS =
			"\x1\xFFFF\x1\x1\x1\x2\x1\x3\x1\x4\x1\x5\x1\x6\x1\x7\x1\x8\x7\xFFFF\x1"+
			"\x13\x1\x14\x1\x15\x1\x16\x1\x17\x1\x18\x1\x19\x1\x5\x1\x9\xA\xFFFF\x1"+
			"\xB\x19\xFFFF\x1\xC\x1\xD\x1\xF\x1\x10\x2\xFFFF\x1\x12\x5\xFFFF\x1\xA"+
			"\x2\xFFFF\x1\xE\x1\x11";
		private const string DFA11_specialS =
			"\x4E\xFFFF}>";
		private static readonly string[] DFA11_transitionS =
			{
				"\x2\x16\x2\xFFFF\x1\x16\x12\xFFFF\x1\x16\x1\x12\x1\x14\x5\xFFFF\x1\x1"+
				"\x1\x2\x2\xFFFF\x1\x3\x1\xFFFF\x1\x4\x1\x15\xA\x13\x1\xFFFF\x1\x5\x1"+
				"\xFFFF\x1\x6\x1\xFFFF\x1\x12\x1\xFFFF\x1A\x10\x1\x11\x3\xFFFF\x1\x10"+
				"\x1\xFFFF\x1\xE\x1\x10\x1\xD\x1\x10\x1\xA\x3\x10\x1\xB\x4\x10\x1\xF"+
				"\x6\x10\x1\xC\x5\x10\x1\x7\x1\xFFFF\x1\x8\x47\xFFFF\x2\x9\x11\xFFFF"+
				"\x1\x9\xC\xFFFF\x2\x9\x11\xFFFF\x1\x9",
				"",
				"",
				"",
				"",
				"",
				"",
				"",
				"",
				"\xA\x10\x2\xFFFF\x1\x10\x1\xFFFF\x1\x10\x2\xFFFF\x1B\x10\x1\xFFFF\x1"+
				"\x10\x1\xFFFF\x1\x10\x1\xFFFF\x1A\x10\x4A\xFFFF\x2\x10\x11\xFFFF\x1"+
				"\x10\xC\xFFFF\x2\x10\x11\xFFFF\x1\x10",
				"\x1\x19",
				"\x1\x1A\x7\xFFFF\x1\x1B",
				"\x1\x1C",
				"\x1\x1E\x2\xFFFF\x1\x1D",
				"\x1\x1F",
				"\x1\x20",
				"",
				"",
				"",
				"",
				"",
				"",
				"",
				"",
				"",
				"\x1\x21\x3\xFFFF\x1\x22",
				"\xA\x10\x2\xFFFF\x1\x10\x1\xFFFF\x1\x10\x2\xFFFF\x1B\x10\x1\xFFFF\x1"+
				"\x10\x1\xFFFF\x1\x10\x1\xFFFF\x1A\x10\x4A\xFFFF\x2\x10\x11\xFFFF\x1"+
				"\x10\xC\xFFFF\x2\x10\x11\xFFFF\x1\x10",
				"\x1\x24",
				"\x1\x25",
				"\x1\x26",
				"\x1\x27",
				"\x1\x28",
				"\x1\x29",
				"\x1\x2A",
				"\x1\x2B",
				"",
				"\x1\x2C",
				"\x1\x2D",
				"\x1\x2E",
				"\x1\x2F",
				"\x1\x30",
				"\x1\x31",
				"\x1\x32",
				"\x1\x33",
				"\x1\x34",
				"\x1\x35",
				"\x1\x36",
				"\x1\x37",
				"\x1\x38",
				"\x1\x39",
				"\x1\x3A",
				"\x1\x3B",
				"\x1\x3C",
				"\xA\x10\x2\xFFFF\x1\x10\x1\xFFFF\x1\x10\x2\xFFFF\x1B\x10\x1\xFFFF\x1"+
				"\x10\x1\xFFFF\x1\x10\x1\xFFFF\x1A\x10\x4A\xFFFF\x2\x10\x11\xFFFF\x1"+
				"\x10\xC\xFFFF\x2\x10\x11\xFFFF\x1\x10",
				"\xA\x10\x2\xFFFF\x1\x10\x1\xFFFF\x1\x10\x2\xFFFF\x1B\x10\x1\xFFFF\x1"+
				"\x10\x1\xFFFF\x1\x10\x1\xFFFF\x1A\x10\x4A\xFFFF\x2\x10\x11\xFFFF\x1"+
				"\x10\xC\xFFFF\x2\x10\x11\xFFFF\x1\x10",
				"\xA\x10\x2\xFFFF\x1\x10\x1\xFFFF\x1\x10\x2\xFFFF\x1B\x10\x1\xFFFF\x1"+
				"\x10\x1\xFFFF\x1\x10\x1\xFFFF\x1A\x10\x4A\xFFFF\x2\x10\x11\xFFFF\x1"+
				"\x10\xC\xFFFF\x2\x10\x11\xFFFF\x1\x10",
				"\xA\x10\x2\xFFFF\x1\x10\x1\xFFFF\x1\x10\x2\xFFFF\x1B\x10\x1\xFFFF\x1"+
				"\x10\x1\xFFFF\x1\x10\x1\xFFFF\x1A\x10\x4A\xFFFF\x2\x10\x11\xFFFF\x1"+
				"\x10\xC\xFFFF\x2\x10\x11\xFFFF\x1\x10",
				"\x1\x41",
				"\x1\x42",
				"\xA\x10\x2\xFFFF\x1\x10\x1\xFFFF\x1\x10\x2\xFFFF\x1B\x10\x1\xFFFF\x1"+
				"\x10\x1\xFFFF\x1\x10\x1\xFFFF\x1A\x10\x4A\xFFFF\x2\x10\x11\xFFFF\x1"+
				"\x10\xC\xFFFF\x2\x10\x11\xFFFF\x1\x10",
				"\x1\x44",
				"",
				"",
				"",
				"",
				"\x1\x45",
				"\x1\x46",
				"",
				"\x1\x47",
				"\x1\x48",
				"\xA\x10\x2\xFFFF\x1\x10\x1\xFFFF\x1\x10\x2\xFFFF\x1B\x10\x1\xFFFF\x1"+
				"\x10\x1\xFFFF\x1\x10\x1\xFFFF\x1A\x10\x4A\xFFFF\x2\x10\x11\xFFFF\x1"+
				"\x10\xC\xFFFF\x2\x10\x11\xFFFF\x1\x10",
				"\x1\x4A",
				"\x1\x4B",
				"",
				"\xA\x10\x2\xFFFF\x1\x10\x1\xFFFF\x1\x10\x2\xFFFF\x1B\x10\x1\xFFFF\x1"+
				"\x10\x1\xFFFF\x1\x10\x1\xFFFF\x1A\x10\x4A\xFFFF\x2\x10\x11\xFFFF\x1"+
				"\x10\xC\xFFFF\x2\x10\x11\xFFFF\x1\x10",
				"\xA\x10\x2\xFFFF\x1\x10\x1\xFFFF\x1\x10\x2\xFFFF\x1B\x10\x1\xFFFF\x1"+
				"\x10\x1\xFFFF\x1\x10\x1\xFFFF\x1A\x10\x4A\xFFFF\x2\x10\x11\xFFFF\x1"+
				"\x10\xC\xFFFF\x2\x10\x11\xFFFF\x1\x10",
				"",
				""
			};

		private static readonly short[] DFA11_eot = DFA.UnpackEncodedString(DFA11_eotS);
		private static readonly short[] DFA11_eof = DFA.UnpackEncodedString(DFA11_eofS);
		private static readonly char[] DFA11_min = DFA.UnpackEncodedStringToUnsignedChars(DFA11_minS);
		private static readonly char[] DFA11_max = DFA.UnpackEncodedStringToUnsignedChars(DFA11_maxS);
		private static readonly short[] DFA11_accept = DFA.UnpackEncodedString(DFA11_acceptS);
		private static readonly short[] DFA11_special = DFA.UnpackEncodedString(DFA11_specialS);
		private static readonly short[][] DFA11_transition;

		static DFA11()
		{
			int numStates = DFA11_transitionS.Length;
			DFA11_transition = new short[numStates][];
			for ( int i=0; i < numStates; i++ )
			{
				DFA11_transition[i] = DFA.UnpackEncodedString(DFA11_transitionS[i]);
			}
		}

		public DFA11( BaseRecognizer recognizer )
		{
			this.recognizer = recognizer;
			this.decisionNumber = 11;
			this.eot = DFA11_eot;
			this.eof = DFA11_eof;
			this.min = DFA11_min;
			this.max = DFA11_max;
			this.accept = DFA11_accept;
			this.special = DFA11_special;
			this.transition = DFA11_transition;
		}

		public override string Description { get { return "1:1: Tokens : ( T__42 | T__43 | T__44 | T__45 | T__46 | T__47 | T__48 | T__49 | NORDIC | EXPLICIT | IF | USING | CONST | INTERFACE | CLASS | ALIAS | NAMESPACE | EXTERN | ID | GUID | Modifier | INT | STRING | COMMENT | WS );"; } }

		public override void Error(NoViableAltException nvae)
		{
			DebugRecognitionException(nvae);
		}
	}

 
	#endregion

}

} // namespace  MessageContracts 
